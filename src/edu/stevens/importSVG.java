...
public class importSVG {
	...
	private int isRootTested = 0;
	public importSVG(File f){
		this.file = f;
		...
		try {
			...
			while(Line != null){
				Pattern pRootStart = Pattern.compile("<svg[^A-Za-z0-9]*");
				...
				if(mRootStart.find()&&isRootTested==0){//parse tag of svg
				...
				if(mGStart.find()){//parse tag of g element
					findGStart = true;
				...
				if(mRectStart.find()){//parse tag of rectangle
				...
				if(mCircleStart.find()){//parse tag of cricle
				...
				if(findPolygonStart||findPolylineStart){//parse tag of polygon or polyline
					...
					if(!pPo.getPoints().equals("none")){//distinguish between polygon and polyline 
	...
	public SVGDocument getDoc(){//get complete SVG file after drawing
		return doc;
	}
